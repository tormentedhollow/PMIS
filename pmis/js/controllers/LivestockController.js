...
app.controller('LivestockController',  function($scope, MFOBannerService, MFOBannerQService){
$scope.isLoading = true;
$scope.toggleSearch = false;

MFOBannerService.async(4).then(function(d){
  if(angular.isUndefined(d.data)) $scope.mfos = [];
  else  $scope.rice = d.data;
  $scope.isLoading = false;
});

MFOBannerQService.async(4).then(function(d){
  if(angular.isUndefined(d.data)) $scope.mfos = [];
 ...
 var jan = Math.round((d.data[0].oba.obl/$scope.ob_all) * 10000)/100;
  ...
  $scope.l_myChart = chart1;
  $scope.l_myChart2 = chart2;
