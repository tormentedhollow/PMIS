...
app.controller('HVCDPController',  function($scope, MFOBannerService, MFOBannerQService){
$scope.isLoading = true;
$scope.toggleSearch = false;

MFOBannerService.async(3).then(function(d){
  if(angular.isUndefined(d.data)) $scope.mfos = [];
  else  $scope.rice = d.data;
  $scope.isLoading = false;
});

MFOBannerQService.async(3).then(function(d){
  if(angular.isUndefined(d.data)) $scope.mfos = [];
  ...
  $scope.h_myChart = chart1;
  $scope.h_myChart2 = chart2;
