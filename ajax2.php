...
function view_ob($mysqli){
	try{
		$mon = $_POST['mon'];
		$query = "SELECT * FROM `tbl_obligation` inner join tbl_mfo on `tbl_obligation`.mfo_id=`tbl_mfo`.mfo_id inner join month on `tbl_obligation`.month=`month`.id
		where month='$mon' and user_id in (select user_id from users where program_id = ".$_SESSION['program_id'].")";
...
function view_oba($mysqli){
	try{
		$mon = $_POST['mon'];
		$query = "SELECT * FROM `tbl_obligationa` inner join tbl_mfo on `tbl_obligationa`.mfo_id=`tbl_mfo`.mfo_id
		where month='$mon' and user_id in (select user_id from users where program_id = ".$_SESSION['program_id'].")";
		$result = $mysqli->query( $query );
		$data = array();
		while ($row = $result->fetch_assoc()) {
...
function getGroup($mysqli){
	try{	
		$query = "SELECT *,  (SELECT COUNT(*) FROM tbl_individual WHERE tbl_individual.group_id = tbl_group.id) total FROM tbl_group where created_by in (select user_id from users where program_id = ".$_SESSION['program_id'].")";
		$result = $mysqli->query( $query );
		$data = array();
		while ($row = $result->fetch_assoc()) {
			$data['data'][] = $row;
...
function getIndividual($mysqli){
	try{
		...
		if(isset($mon)&&$mon!=''){
			...
			inner join `tbl_group` on tbl_individual.group_id = tbl_group.id inner join `month` on tbl_individual.month = month.id where month='$mon'";	
		}
		if(isset($mon)&&$mon!=''){
			$query = "SELECT tbl_individual.*, tbl_group.name, month.month_name  FROM `tbl_individual` 
			inner join `tbl_group` on tbl_individual.group_id = tbl_group.id inner join `month` on tbl_individual.month = month.id where group_id='$gid'";	
		}
		$result = $mysqli->query( $query );
		$data = array();
		while ($row = $result->fetch_assoc()) {
...
function getPhysical($mysqli){
	...
	try{
		for($i=2;$i<count($unit);$i++){
			...
			$query = "SELECT sum(kilo) as k1 FROM `tbl_registered` WHERE `mfo_id` in (select mfo_id FROM mfo where unitofmeasure='$i')";
			$result = $mysqli->query( $query );
			...
			$query = "SELECT sum(kilo) as q1 FROM `tbl_registereda` WHERE `mfo_id` in (select mfo_id FROM mfo where unitofmeasure='$i') and (month=1 or month=2 or month=3)";
			$result = $mysqli->query( $query );
			$temp['q1'] = $result->fetch_assoc();

			$query = "SELECT sum(kilo) as q2 FROM `tbl_registereda` WHERE `mfo_id` in (select mfo_id FROM mfo where unitofmeasure='$i') and (month=4 or month=5 or month=6)";
			$result = $mysqli->query( $query );
			$temp['q2'] = $result->fetch_assoc();

			$query = "SELECT sum(kilo) as qt FROM `tbl_registereda` WHERE `mfo_id` in (select mfo_id FROM mfo where unitofmeasure='$i') and (month=1 or month=2 or month=3 or month=4 or month=5 or month=6)";
			$result = $mysqli->query( $query );
			...
			for($x=1;$x<6;$x++){
				$query = "SELECT sum(kilo) as k1 FROM `tbl_registered` WHERE `mfo_id` in (select mfo_id FROM mfo where unitofmeasure='$i') and user_id in (select user_id from users where program_id = '$x')";
				$result = $mysqli->query( $query );
				$row['k1'] = $result->fetch_assoc();

				$query = "SELECT sum(kilo) as q1 FROM `tbl_registereda` WHERE `mfo_id` in (select mfo_id FROM mfo where unitofmeasure='$i') and user_id in (select user_id from users where program_id = '$x') and (month=1 or month=2 or month=3)";
				$result = $mysqli->query( $query );
				$row['q1'] = $result->fetch_assoc();

				$query = "SELECT sum(kilo) as q2 FROM `tbl_registereda` WHERE `mfo_id` in (select mfo_id FROM mfo where unitofmeasure='$i') and user_id in (select user_id from users where program_id = '$x') and (month=4 or month=5 or month=6)";
				$result = $mysqli->query( $query );
				$row['q2'] = $result->fetch_assoc();

				$query = "SELECT sum(kilo) as qt FROM `tbl_registereda` WHERE `mfo_id` in (select mfo_id FROM mfo where unitofmeasure='$i') and user_id in (select user_id from users where program_id = '$x') and (month=1 or month=2 or month=3 or month=4 or month=5 or month=6)";
				$result = $mysqli->query( $query );
...
function getPhysicalMFO($mysqli){ //getByMFO
	try{	
		$query = "SELECT DISTINCT(header_id) FROM `tbl_registered` where user_id in (select user_id from users where program_id = ".$_SESSION['program_id'].")";
		$result = $mysqli->query( $query );
		$data = array();
		...
		while ($row = $result->fetch_assoc()) {
			...
			where user_id in (select user_id from users where program_id = ".$_SESSION['program_id'].")";
			$resultUnit = $mysqli->query( $queryUnit );
			while ($rowUnit = $resultUnit->fetch_assoc()){
				$q = "SELECT sum(kilo) as k FROM `tbl_registered` where unit_id='$rowUnit[unit_id]' and header_id='$temp[header_id]' and user_id in (select user_id from users where program_id = ".$_SESSION['program_id'].")";
				$r= $mysqli->query($q);
				$row['annual'] = $r->fetch_assoc()['k'];

				$row['header_id'] = $temp['header_id'];

				$q = "SELECT sum(kilo) as k FROM `tbl_registered` where unit_id='$rowUnit[unit_id]' and header_id='$temp[header_id]' and month='1' and user_id in (select user_id from users where program_id = ".$_SESSION['program_id'].")";
				$r= $mysqli->query($q);
				$row['jan'] = $r->fetch_assoc()['k'];

				$q = "SELECT sum(kilo) as k FROM `tbl_registered` where unit_id='$rowUnit[unit_id]' and header_id='$temp[header_id]' and month='2' and user_id in (select user_id from users where program_id = ".$_SESSION['program_id'].")";
				$r= $mysqli->query($q);
				$row['feb'] = $r->fetch_assoc()['k'];

				$q = "SELECT sum(kilo) as k FROM `tbl_registered` where unit_id='$rowUnit[unit_id]' and header_id='$temp[header_id]' and month='3' and user_id in (select user_id from users where program_id = ".$_SESSION['program_id'].")";
				$r= $mysqli->query($q);
				$row['mar'] = $r->fetch_assoc()['k'];

				$q = "SELECT sum(kilo) as k FROM `tbl_registered` where unit_id='$rowUnit[unit_id]' and header_id='$temp[header_id]' and month='4' and user_id in (select user_id from users where program_id = ".$_SESSION['program_id'].")";
				$r= $mysqli->query($q);
				$row['apr'] = $r->fetch_assoc()['k'];

				$q = "SELECT sum(kilo) as k FROM `tbl_registered` where unit_id='$rowUnit[unit_id]' and header_id='$temp[header_id]' and month='5' and user_id in (select user_id from users where program_id = ".$_SESSION['program_id'].")";
				$r= $mysqli->query($q);
				$row['may'] = $r->fetch_assoc()['k'];

				$q = "SELECT sum(kilo) as k FROM `tbl_registered` where unit_id='$rowUnit[unit_id]' and header_id='$temp[header_id]' and month='6' and user_id in (select user_id from users where program_id = ".$_SESSION['program_id'].")";
				$r= $mysqli->query($q);
				$row['jun'] = $r->fetch_assoc()['k'];
...
function add_kilo($mysqli){
	try{
		...
		if($a){
			$query = "INSERT INTO tbl_registereda(year, month, province, municipal, kilo, mfo_id, user_id) 
			VALUES ('2015', '$month', '$provs', '$muns', '$kilo', '$mfo_id', ".$_SESSION['user_id'].")";
		}else{
			$query = "INSERT INTO tbl_registered(year, month, province, municipal, kilo, mfo_id, user_id, financial, header_id, unit_id)
			 VALUES ('2015', '$month', '$provs', '$muns', '$kilo', '$mfo_id', ".$_SESSION['user_id']." ,'$fin','$header_id', '$unit')";					
		}
...
function add_individual($mysqli){
	try{
		...
		$query = "INSERT INTO tbl_individual(surName, firstName, middleName, sex, contact, email, province, municipal, barangay, month, group_id, created_by, mfo_id) 
		VALUES ('$sn', '$fn', '$mn', '$sex', '$contact', '$email', '$provs', '$muns', '$brgy', '$month', '$group', ".$_SESSION['user_id'].", '$mfo_id')";
...
function add_ob($mysqli){
	try{
		...
		$query = "SELECT * FROM tbl_obligation where mfo_id='$mfo_id' and month = '$month'";
		$result = $mysqli->query($query);
...
function add_oba($mysqli){
	try{
		...
		$query = "SELECT * FROM tbl_obligationa where mfo_id='$mfo_id' and month = '$month'";
		$result = $mysqli->query($query);
		if($result->num_rows>0)
